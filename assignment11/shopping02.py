...
async def checkout_customer(queue: Queue, cashier_number: int):
    ...
    while not queue.empty():
        ...
        customer_count += 1 #เพิ่มจำนวนลูกค้าที่แคชเชียร์ได้ทำการตรวจสินค้าเสร็จ
        total_checkout_time += time.perf_counter() - customer_start_time # คำนวนเวลารวม
...
async def main():
    ...
    for i, (customer_count, total_checkout_time) in enumerate(results[1:], start=0):
        print(f"The Cashier_{i} took {customer_count} customers "
              f"total {round(total_checkout_time, ndigits=2)} secs.")
    print("---------------------------------------------------")
